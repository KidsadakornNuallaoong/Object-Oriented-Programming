...
int ArrayList::remove(int i1,int i2)
{
    // * check lower
    if(curSize == 0){
        cout << "[Massage] List is empty!!!" << endl;
        return -1;
    }else if (i1 < 0 && i2 < 0 )
    {
        cout << "[Massage Warning] : Can't remove element this lower bound" << endl;
        return -1;
    }else if(i1 >= curSize && i2 >= curSize){
        // * check upper
        cout << "[Massage Warning] : Can't remove element this over bound" << endl;
        return -1;
    }
    // * input 2 index
    // * remove index {i1 to i2 element}
    int i = 0;
    int j;
    // * 2 loop while loop 2 round i = 0 +... -> i2-i1
    while (i <= (i2-i1)){
        // * loop shift element
        for(j = i1; j <= curSize-1; ++j){
            L[j] = L[j+1];
        }
        // * after shift element -> down curSize from this round
        curSize--;
        i++;
    }
    // * counting n of index
